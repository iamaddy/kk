...
define(function(require, exports, module){
	...
	ScrollBar.prototype = {
		initBar:function(options){
			...
			this.content = d(this.options.barContent)[0];//文档中的真实内容
            this.scrollObj = d(this.options.scrollContent)[0];//实际可见的展示区域
			this.bar = d(this.options.barId)[0];
			...
			//绑定鼠标事件
			et.add(this.bar, 'mousedown', events.onMouseDown);
			et.add(doc, 'mousemove', events.onMouseMove);
			et.add(doc, 'mouseup', events.onMouseUp);
			et.add(this.content, 'mousewheel', events.onMouseWheel);
            et.add(this.content, 'wheel', events.onWheel);
			et.add(this.content, 'dommousescroll', events.onDomMouseScroll);
		...
		setBarHeight: function() {
			...
			this.scrollObj.offsetHeight - this.content.scrollHeight >= 0 ? (this.bar.t = 0) : (this.bar.style.height = parseInt(this.scrollObj.offsetHeight / this.content.scrollHeight * this.scrollObj.offsetHeight) + "px",this.bar.style.display = '',this.bar.t = parseInt(this.bar.style.marginTop));
			this.scroll(0);
		}
	}
